# Curiosity Log — 2026-07-23


## Curiosity — 00:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🔀 **Intersection:** llm inference × self hosting
  • Should You Self-Host LLM Inference? Cost and Risk Guide
    https://theaiengineer.substack.com/p/should-you-self-host-inference
📄 **arXiv (ai agents):**
  • FMRP-LEAN: A HIPAA-Compliant AI-Augmented LIMS Architecture for End-to-End Clini
    http://arxiv.org/abs/2607.20382v1
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐21925
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐12969
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • deepseek-ai/DeepSpec ⭐6743
    DeepSpec: a full-stack codebase for training and evaluating speculative decoding
    https://github.com/deepseek-ai/DeepSpec
🎲 **Serendipity:**
  • Businesses with ugly AI menu redesigns
    https://blog.fiddery.com/businesses-with-ugly-ai-menu-redesigns/
    (252 points on HN)

## Curiosity — 12:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to self hosting:** nixos
  • Download | Nix & NixOS
    https://nixos.org/download/
  • 下载 | Nix & NixOS 发行版
    https://nixos.cn/download/
🌱 **Adjacent to ai agents:** constitutional ai
  • CONSTITUTIONAL Simple Definition - Merriam-Webster
    https://www.merriam-webster.com/simple/constitutional
🔀 **Intersection:** ai agents × monitoring
  • ‎Google Gemini
    https://gemini.google.com/
📄 **arXiv (llm inference):**
  • Lipschitzian SLLNs for random functions
    http://arxiv.org/abs/2607.20411v1
  • Towards Miniature Humanoid Tele-Loco-Manipulation Using Virtual Reality and Rein
    http://arxiv.org/abs/2607.20399v1
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐22054
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐13024
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • unicity-aos/aos-ce ⭐6831
    AOS Community Edition: the open agent operating system.
    https://github.com/unicity-aos/aos-ce
🎲 **Serendipity:**
  • DARPA, U.S. Air Force fly AI-controlled F-16
    https://www.darpa.mil/news/2026/darpa-us-air-force-fly-ai-controlled-f-16
    (73 points on HN)

## Curiosity — 16:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to self hosting:** nomad
  • High Performance Hunting Clothes & Gear | Nomad Outdoor
    https://nomadoutdoor.com/
🔀 **Intersection:** ai agents × self hosting
📄 **arXiv (llm inference):**
  • PG-KINN: A Physics-Informed Petrov-Galerkin Kolmogorov-Arnold Network for Solvin
    http://arxiv.org/abs/2607.20378v1
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐22077
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐13042
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • unicity-aos/aos-ce ⭐6863
    AOS Community Edition: the open agent operating system.
    https://github.com/unicity-aos/aos-ce
🎲 **Serendipity:**
  • DARPA, U.S. Air Force fly AI-controlled F-16
    https://www.darpa.mil/news/2026/darpa-us-air-force-fly-ai-controlled-f-16
    (153 points on HN)

## Curiosity — 20:18 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to self hosting:** nomad
  • 某安卓平台3d绘图工具的简单逆向 - 吾爱破解 - 52pojie.cn
    https://www.52pojie.cn/thread-1820367-1-1.html
  • Digital Nomad in 2026: The Complete Guide to Working & Living Abroad
    https://hellosafe.com/digital-nomad
🔀 **Intersection:** llm inference × ai agents
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐22134
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐13060
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • unicity-aos/aos-ce ⭐6899
    AOS Community Edition: the open agent operating system.
    https://github.com/unicity-aos/aos-ce
🎲 **Serendipity:**
  • AI Companies Are Trying to Hide a Staggering Amount of Debt
    https://futurism.com/artificial-intelligence/ai-companies-hide-debt-off-balance-sheet
    (620 points on HN)
