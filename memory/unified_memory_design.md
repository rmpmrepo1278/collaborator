...
## Decision: single Unified Memory MCP server
 (stdio) fronts ALL stores behind 4 tools:
-  — SQL/search across stores
-  — write (observations, facts, KG capsules, goals, tasks, collab notes)
-  — cross-store recall
-  — store sizes
...
Hermes internal scripts may keep using DBs directly (safe). New code should import
 client. Full rewrite of ingestion (conversation_processor, mind_loop) is
